 ...
 * Handles demo request submissions, validation, anti-spam, storage, and Resend email notification.
 */
...
export async function onRequestPost(context) {
  ...
  // 2. Anti-spam: Honeypot check (field 'website' must be empty)
  ...
  // 3. Anti-spam: Timestamp check (at least 3 seconds from page load)
  const now = Date.now();
  const pageLoadTime = parseInt(timestamp, 10);
  if (!pageLoadTime || (now - pageLoadTime) < 3000) {
    // Submitted suspiciously fast (< 3 seconds) -> silent discard
    return new Response(JSON.stringify({ success: true, message: "Díky, ozvu se do 24 hodin." }), {
      status: 200,
      headers: { "Content-Type": "application/json; charset=utf-8" },
    });
  ...
  // 5. Rate Limiting (Check KV if bound)
  ...
  // 6. Persistence: Always save the lead
  const leadRecord = {
  ...
  // 7. Email Notification via Resend API
  if (env.RESEND_API_KEY) {
    ...
    console.log("RESEND_API_KEY not configured. Lead recorded locally:", leadRecord);
  }
